...
function typeBadgeClass(type: Alert['type']): string {
  if (type === 'low_stock')   return 'bg-orange-100 text-orange-700 border border-orange-200';
  if (type === 'near_expiry') return 'bg-yellow-100 text-yellow-700 border border-yellow-200';
...
function cardBgClass(severity: Alert['severity'], read: boolean): string {
  if (read) return 'bg-white border border-slate-200';
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Alert Centre</h1>
      ...
      <div className="mb-5 flex items-start gap-3 bg-blue-50 border border-blue-200 rounded-xl px-4 py-3">
        ...
        <div>
          <span className="text-sm font-semibold text-blue-800">Smart Throttling Active</span>
      ...
      {filtered.length === 0 ? (
        <div className="bg-white border border-slate-200 rounded-xl p-12 text-center shadow-sm">
          {alerts.length === 0 ? (
            ...
            <>
              <Info size={28} className="mx-auto text-slate-300 mb-3" />
        ...
        <div className="space-y-2.5">
          {filtered.map(alert => {
            ...
            return (
              <div
                key={alert.id}
                className={`rounded-xl overflow-hidden shadow-sm transition-all ${cardBgClass(alert.severity, alert.read)}`}
              >
                {/* Main row */}
                <div
                    ...
                    <div className="flex items-start justify-between gap-3 flex-wrap">
                      <div className="flex items-start gap-3 flex-1 min-w-0">
                        ...
                        <div className="min-w-0 flex-1">
                          {/* Top row: type badge + drug name + unread dot */}
                          ...
                          {/* Message */}
                          <p className={`text-xs text-slate-600 leading-relaxed ${!isExpanded ? 'truncate' : ''}`}>
                            {alert.message}
                          </p>
                        </div>
                      </div>
                ...
                {isExpanded && (
                  <div className="border-t border-slate-200/70 bg-white/60 px-5 py-4 ml-1">
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-4 text-xs">
                      <div>
                        <p className="text-slate-400 mb-0.5">Drug</p>
                        <p className="font-semibold text-slate-700">{alert.drugName}</p>
                      </div>
                      <div>
                    ...
                    <p className="text-xs text-slate-600 bg-slate-50 border border-slate-100 rounded-lg px-3 py-2 mb-4 leading-relaxed">
                      {alert.message}
                    </p>
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <button
                        ...
                        className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold border border-slate-200 text-slate-600 rounded-lg hover:bg-slate-50 transition-colors"
                      >
                        <X size={13} /> Dismiss
                      </button>
                      {alert.read && (
      ...
      <div className="mt-6 bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
        ...
        {showStats && (
          <div className="px-5 pb-5 border-t border-slate-100">
            {alerts.length === 0 ? (
              <p className="text-slate-400 text-sm py-6 text-center">No alert data yet.</p>
            ) : (
            ...
            <div className="mt-5 grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                { label: 'Total (7d)',    value: last7.reduce((s, d) => s + countForDay(d, 'critical') + countForDay(d, 'warning'), 0), color: 'text-slate-700' },
                { label: 'Critical (7d)', value: last7.reduce((s, d) => s + countForDay(d, 'critical'), 0), color: 'text-red-600' },
                { label: 'Warning (7d)',  value: last7.reduce((s, d) => s + countForDay(d, 'warning'),  0), color: 'text-amber-600' },
                { label: 'Auto-resolved', value: alerts.filter(a => a.read).length, color: 'text-emerald-600' },
